...
EOF
    ...
    if [[ "$host" == "kubemaster1" ]]; then
        ...
        kubeadm init --upload-certs --control-plane-endpoint "lb.example.com:6443" --pod-network-cidr=10.244.0.0/16 --apiserver-advertise-address=192.168.1.201 --certificate-key=$ClusterJoinCertkey_Master
...
EOF
    ...
    echo "[3. Installing Containerd as a container runtime engine.]"
    apt-get update -o Acquire::CompressionTypes::Order::=gz  ; apt-get remove docker  docker.io containerd runc
    apt-get -y install \
    apt-transport-https \
    ca-certificates \
    curl \
